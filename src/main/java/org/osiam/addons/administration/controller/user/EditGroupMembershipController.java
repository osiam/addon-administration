...
public class EditGroupMembershipController extends GenericController {
    ...
    public ModelAndView handleEditGroupMembership(
        ...
        modelAndView.addObject(MODEL_PAGING_INFORMATION_UNASSIGNED_GROUPS,
                session.getUnassignedGroupsPagingInformation());
    ...
    private RedirectBuilder defaultRedirect(String userId) {
        return new RedirectBuilder()
                ...
                .addParameter(REQUEST_PARAMETER_ASSIGNED_OFFSET,
                        session.getAssignedGroupsPagingInformation().getOffset())
                .addParameter(REQUEST_PARAMETER_ASSIGNED_ORDER_BY,
                        session.getAssignedGroupsPagingInformation().getOrderBy())
                .addParameter(REQUEST_PARAMETER_ASSIGNED_ASCENDING,
                        session.getAssignedGroupsPagingInformation().getAscending())
                .addParameter(REQUEST_PARAMETER_UNASSIGNED_QUERY,
                        session.getUnassignedGroupsPagingInformation().getQuery())
                .addParameter(REQUEST_PARAMETER_UNASSIGNED_LIMIT,
                        session.getUnassignedGroupsPagingInformation().getLimit())
                .addParameter(REQUEST_PARAMETER_UNASSIGNED_OFFSET,
                        session.getUnassignedGroupsPagingInformation().getOffset())
                .addParameter(REQUEST_PARAMETER_UNASSIGNED_ORDER_BY,
                        session.getUnassignedGroupsPagingInformation().getOrderBy())
                .addParameter(REQUEST_PARAMETER_UNASSIGNED_ASCENDING,
                        session.getUnassignedGroupsPagingInformation().getAscending());
    }
}
