...
public class EditUserMembershipController extends GenericController {
    ...
    public ModelAndView handleEditUserMembership(@RequestParam(value = REQUEST_PARAMETER_ID) final String groupId,
        ...
        modelAndView
                .addObject(MODEL_PAGING_INFORMATION_UNASSIGNED_USERS, session.getUnassignedUsersPagingInformation());
    ...
    private RedirectBuilder defaultRedirect(String groupId) {
        return new RedirectBuilder()
                ...
                .addParameter(REQUEST_PARAMETER_ASSIGNED_OFFSET,
                        session.getAssignedUsersPagingInformation().getOffset())
                .addParameter(REQUEST_PARAMETER_ASSIGNED_ORDER_BY,
                        session.getAssignedUsersPagingInformation().getOrderBy())
                .addParameter(REQUEST_PARAMETER_ASSIGNED_ASCENDING,
                        session.getAssignedUsersPagingInformation().getAscending())
                .addParameter(REQUEST_PARAMETER_UNASSIGNED_QUERY,
                        session.getUnassignedUsersPagingInformation().getQuery())
                .addParameter(REQUEST_PARAMETER_UNASSIGNED_LIMIT,
                        session.getUnassignedUsersPagingInformation().getLimit())
                .addParameter(REQUEST_PARAMETER_UNASSIGNED_OFFSET,
                        session.getUnassignedUsersPagingInformation().getOffset())
                .addParameter(REQUEST_PARAMETER_UNASSIGNED_ORDER_BY,
                        session.getUnassignedUsersPagingInformation().getOrderBy())
                .addParameter(REQUEST_PARAMETER_UNASSIGNED_ASCENDING,
                        session.getUnassignedUsersPagingInformation().getAscending());
    }
}
